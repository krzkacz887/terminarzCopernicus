...
def init_db():
    """Tworzy bazę danych i tabelę pracowników z obsługą konkretnych dni miesiąca."""
...
class TabelaGrafikApp(ctk.CTk):
    ...
    def _wczytaj_pracownikow_z_bazy(self):
        ...
        if not rekordy:
            wszystkie_dni = ",".join(map(str, range(1, 32)))
            domyslne = [
                ("Jan", "Kowalski", 1.0, wszystkie_dni),
                ("Anna", "Nowak", 1.0, wszystkie_dni),
                ("Piotr", "Wiśniewski", 0.5, wszystkie_dni),
                ("Katarzyna", "Wójcik", 0.75, wszystkie_dni)
            ]
    ...
    def _otworz_edycje_pracownika(self, pracownik: Pracownik):
        max_dni = len(self.obj_miesiac.dni)
        okno = OknoEdycjiPracownika(self, pracownik, max_dni_miesiaca=max_dni)
    ...
    def dodaj_pracownika(self):
        max_dni = len(self.obj_miesiac.dni)
        okno = OknoEdycjiPracownika(self, max_dni_miesiaca=max_dni)
        self.wait_window(okno)
        if okno.wynik:
            self.pracownicy = self._wczytaj_pracownikow_z_bazy()
            self._wygeneruj_tabele()
    ...
    def automatycznie_generuj_grafik(self):
        try:
            generator = GeneratorGrafiku(self.pracownicy, self.obj_miesiac, self.norma_miesiaca)
            generator.wygeneruj()

            for (r, c), (prac, dzien_obj) in self.dane_pól.items():
                stan = dzien_obj.pobierz_zmiane(prac.db_id)
                self.pola[(r, c)].set(stan)
                self._odswiez_kolor_komorki(r, c)

            self.przelicz_sumy()
            messagebox.showinfo("Sukces", "Automatyczny grafik został wygenerowany.", parent=self)
        except Exception as e:
            messagebox.showerror("Błąd generatora", f"Wystąpił błąd podczas generowania grafiku:\n{e}", parent=self)
...
class GeneratorGrafiku:
    ...
    uwzględnia dozwolone dni pracy pracowników i uzupełnia wyłącznie pola z "P".
    """
    ...
    def _oblicz_godziny_zmiany(self, status: str, pracownik) -> float:
        if status in ["D", "N"]:
            return 12.0
        elif status == "M":
            return 8.0
    ...
    def generuj(self) -> bool:
        ...
        for idx, dzien in enumerate(self.miesiac.dni):
            ...
            # Sortujemy pracowników z największym niedoborem godzin
            dostepni_pracownicy = sorted(
                self.pracownicy,
                key=lambda p: (godziny_pracownikow[p.db_id] / p.get_norma_miesiaca(self.norma_miesiaca))
            )
            ...
            # --- Krok A: Dopisywanie 'D' ---
            if not oblozenie_d and suma_godzin_dnia <= 12.0:
            ...
            # --- Krok B: Dopisywanie 'N' ---
            if not oblozenie_n and suma_godzin_dnia <= 12.0:
                for p in dostepni_pracownicy:
                    if dzien.pobierz_zmiane(p.db_id) != "P":
                        continue
                    if not p.moze_pracowac_w_dzien(dzien.data):
